...
class Compiler:
    ...
    root_stack = 0
    used_callee = []
    unused_callee = [Reg("rsp"), Reg("rbp"), Reg("rbx"), Reg("r12"), Reg("r13"), Reg("r14"), Reg("r15")]
    varTypes = {}
    tuple_lengths = {}
    tupleColors = []
    varColors = []
    ...
    def shrink_exp(self, s: expr) -> expr:
          match s:
                ...
                case ast.List(es, Load()):
                    ...
                    for e in es:
                         es_s.append(self.shrink_exp(e))
                    return ast.List(es_s, Load())
                case Subscript(tup, index, Load()): # can be used to address both Tuples and List
                ...
                case Allocate(length, typ):
                    return s
                case AllocateArray(length, typ): # should this be handled here ? 
                    return s
    ...
    def shrink_stmt(self, s: stmt) -> stmt:
        match s:
            ...
            case Collect(size):
                    return s
            case Assign([Subscript(tup, index, Store())], value): #can be used to address both Tuples and Arrays
    ...
    def resolve_exp(self, s: expr) -> expr:
        match s:
              case IfExp(test, body, orelse):
                    res = self.resolve_exp(test)
                    body_exp = self.resolve_exp(body)
                    or_exp = self.resolve_exp(orelse)
                    return IfExp(res, body_exp, or_exp)
              case UnaryOp(Not(), v):
                    val = self.resolve_exp(v)
                    return (UnaryOp(Not(), val))
              case UnaryOp(USub(), v):
                    val = self.resolve_exp(v)
                    return UnaryOp(USub(), v)
              case BinOp(left, Add(), right):
                    l = self.resolve_exp(left)
                    r = self.resolve_exp(right)
                    return BinOp(l, Add(), r)
              case BinOp(left, Sub(), right):
                    l = self.resolve_exp(left)
                    r = self.resolve_exp(right)
              ...
              case Tuple(es, Load()):
                    ...
                    es_s = []
                    for e in es:
                         es_s.append(self.resolve_exp(e))
                    return Tuple(es_s, Load())
              case ast.List(es, Load()):
                    es_s = []
                    for e in es:
                    ...
                    return ast.List(es_s, Load())
              case Subscript(tup, index, Load()): # can be used to address both Tuples and List
                    print("calling resolve on args-", type(tup), type(index)) 
                    t_s = self.resolve_exp(tup)
                    n_s = self.resolve_exp(index)
                    if type(t_s) == ListType:
                         return Call(Name('array_load'), [t_s, n_s])
                    else:
                         return Subscript(t_s, n_s, Load())
              case Call(Name('len'), [tup]):
                    t_s = self.resolve_exp(tup)
                    if type(t_s) == ListType:
                         return Call(Name('array_len'), [t_s])
                    else:
                         return Call(Name('len'), [t_s])
              case _ :
                    return s
    ...
    def resolve_stmt(self, s: stmt) -> stmt:
        match s:
            case If(test, body, orelse):
                ...
                body_exp = []
                for b in body:
                     body_exp.append(self.resolve_stmt(b))
                or_exp = []
                for o in orelse:
                     or_exp.append(self.resolve_stmt(o))
                return If(res, body_exp, or_exp)
            case Expr(Call(Name('print'), [arg])):
                val = self.resolve_stmt(arg)
                return Expr(Call(Name('print'), [val]))
            case Expr(value):
                val = self.resolve_stmt(value)
                return Expr(val)
            case Assign([Name(id)], value):
                val = self.resolve_stmt(value)
                return Assign([Name(id)], val)
            case While(exp, stmts, []):
                  cond_shrink = self.resolve_stmt(exp)
                  stmts_shrink = []
                  for x in stmts:
                    stmts_shrink.append(self.resolve_stmt(x))
                  return While(cond_shrink, stmts_shrink, [])
            case Collect(size):
                    return s
            case Assign([Subscript(tup, index, Store())], value):
                    tup_s = self.resolve_exp(tup)
                    index_s = self.resolve_exp(index)
                    val_s = self.resolve_exp(value)
                    if type(tup_s) == List:
                         return Call(Name('array_store'), [tup_s, index_s, val_s])
                    else:
                         return Assign([Subscript(tup_s, index_s, Store())], val_s)
            case _:
                return self.resolve_exp(s)
    ...
    def resolve(self, p):
            match p:
                case Module(body):
                    res = [] 
                    for x in body:
                        res.append(self.resolve_stmt(x))
                    return Module(res)
                case _ :
                    raise Exception('error in resolve main module, unexpected ' + repr(p))
    ...
    def check_bounds_exp(self, s: expr) -> expr:
        match s:
              case IfExp(test, body, orelse):
                    res = self.check_bounds_exp(test)
                    body_exp = self.check_bounds_exp(body)
                    or_exp = self.check_bounds_exp(orelse)
                    return IfExp(res, body_exp, or_exp)
              case UnaryOp(Not(), v):
                    val = self.check_bounds_exp(v)
                    return (UnaryOp(Not(), val))
              case UnaryOp(USub(), v):
                    val = self.check_bounds_exp(v)
                    return UnaryOp(USub(), v)
              case BinOp(left, Add(), right):
                    l = self.check_bounds_exp(left)
                    r = self.check_bounds_exp(right)
                    return BinOp(l, Add(), r)
              case BinOp(left, Sub(), right):
                    l = self.check_bounds_exp(left)
                    r = self.check_bounds_exp(right)
              ...
              case Tuple(es, Load()):
                    ...
                    es_s = []
                    for e in es:
                         es_s.append(self.check_bounds_exp(e))
                    return Tuple(es_s, Load())
              case ast.List(es, Load()):
                    es_s = []
                    for e in es:
              ...
              case Call(Name('array_load'), [tup, index]):
                    if type(index) == int and (index < 0 or index >= self.tuple_lengths[tup] ):
    ...
    def check_bounds_stmt(self, s: stmt) -> stmt:
        match s:
            case If(test, body, orelse):
                ...
                body_exp = []
                for b in body:
                     body_exp.append(self.check_bounds_stmt(b))
                or_exp = []
                for o in orelse:
                     or_exp.append(self.check_bounds_stmt(o))
                return If(res, body_exp, or_exp)
            case Expr(Call(Name('print'), [arg])):
                val = self.check_bounds_exp(arg)
                return Expr(Call(Name('print'), [val]))
            case Expr(value):
                val = self.check_bounds_exp(value)
                return Expr(val)
            case Assign([Name(id)], value):
                val = self.check_bounds_exp(value)
                return Assign([Name(id)], val)
            case While(exp, stmts, []):
                  cond_shrink = self.check_bounds_exp(exp)
                  stmts_shrink = []
                  for x in stmts:
                    stmts_shrink.append(self.check_bounds_stmt(x))
                  return While(cond_shrink, stmts_shrink, [])
            case Collect(size):
                    return s
            case Assign([Subscript(tup, index, Store())], value):
            ...
            case Call(Name('array_store'), [tup, index, value]):
                  if type(index) == int and (index < 0 or index >= self.tuple_lengths[tup]):
                       ...
                       return s
            case _:
                return self.check_bounds_exp(s)
    ...
    def check_bounds(self, p):
            match p:
                case Module(body):
                    res = [] 
                    for x in body:
                        res.append(self.check_bounds_stmt(x))
                    return Module(res)
                case _ :
                    raise Exception('error in check bounds main module, unexpected ' + repr(p))
    ...
    def expose_alloc_exp(self, e: expr) -> expr:
            match e:
                ...
                case ast.List(es, Load()):
                    ...
                    begin_stms.append(If(Compare(BinOp(self.free_ptr,Add(),Constant(bytes_req)),[Lt()],[self.fromspace_end]), [], [Collect(bytes_req)]))
                    tuple = generate_name('alloc')
                    begin_stms.append(Assign([Name(tuple)],AllocateArray(t_len, e.has_type)))
                    print("Type of has Type var-", type(e.has_type))
                    for ind in range(len(temp_var)):
                         begin_stms.append(Call(Name('array_store'), [Name(tuple), Constant(ind), Name(temp_var[ind])]))
                    return Begin(begin_stms, Name(tuple))
                ...
                case Call(Name('array_load'), [tup, index]):
                    tup_rco = self.expose_alloc_exp(tup)
                    ind_rco = self.expose_alloc_exp(index)
                    return Call(Name('array_load'), [tup_rco, ind_rco])
                case Call(Name('len'), [tup]):
                    tup_rco = self.expose_alloc_exp(tup)
                    return Call(Name('len'), [tup_rco])
                case Call(Name('array_len'), [t_s]):
                      tup_rco = self.expose_alloc_exp(tup)
                      return Call(Name('array_len'), [tup_rco])
                case Allocate(length, typ): # should this be handled here ? 
                    return e
                case AllocateArray(length, typ): # should this be handled here ? 
                    return e
                case GlobalValue(name):
    ...
    def expose_alloc_stmt(self, s: stmt) -> List[stmt]:
        ...
        match s:
            ...
            case Call(Name('array_store'), [tup, index, value]):
                  a_atm = self.expose_alloc_exp(tup)
                  i_atm = self.expose_alloc_exp(index)
                  val_atm = self.expose_alloc_exp(value)
                  res.append(Call(Name('array_store'), [a_atm, i_atm, val_atm]))
            case _:
            ...
            match e:
                ...
                case BinOp(left, Mult(), right):
                    ...
                    if need_atomic:
                         ...
                         for y in r[1]:
                              temp.append(y)
                         temp.append((Name(var),BinOp(l[0] , Mult(), r[0])))
                         return (Name(var), temp)
                    else:
                         return (BinOp(l[0], Mult(), r[0]),[])
                case UnaryOp(USub(), v):
                    ...
                    if need_atomic:
                         ...
                         return (UnaryOp(USub(), val[0]), val[1])
                case IfExp(test, body, orelse):
                ...
                case Call(Name('array_load'), [tup, index]):
                    ...
                    if need_atomic:
                         var = generate_name("temp")
                         temp.append((Name(var), Call(Name('array_load'), [tup_rco[0], ind_rco[0]])))
                         return (Name(var), temp)
                    else:
                         return (Call(Name('array_load'), [tup_rco[0], ind_rco[0]]), temp)        
                case Call(Name('len'), [tup]):
                    tup_rco = self.rco_exp(tup, True)
                    temp = []
                    for x in tup_rco[1]:
                ...
                case Call(Name('array_len'), [tup]):
                    ...
                    if need_atomic:
                         var = generate_name("temp")
                         temp.append((Name(var), Call(Name('array_len'), [tup_rco[0]])))
                         return (Name(var), temp)
                    else:
                         return (Call(Name('array_len'), [tup_rco[0]]),temp)  
                case Allocate(length, typ):
                    return (Allocate(length, typ),[])
                case AllocateArray(length, typ):
                    # we create the node AllocateArray with length as int ? 
                    ...
                    if need_atomic:
                         var = generate_name("temp")
                         temp.append((Name(var), AllocateArray(len_rco[0], typ)))
                         return (Name(var), temp)
                    else:
                         return (AllocateArray(len_rco[0], typ),temp) 
                case GlobalValue(name):
                    return (e,[])
                case _:
    ...
    def rco_stmt(self, s: stmt) -> List[stmt]:
        ...
        match s:
            ...
            case Call(Name('array_store'), [tup, index, value]):
                  ...
                  for i in val_atm[1]:
                       res.append(i)
                  res.append(Call(Name('array_store'), [t_atm[0], i_atm[0], val_atm[0]]))
            case _:
    ...
    def select_exp(self, e: expr) -> List[instr]:
         ...
         match e:
            ...
            case BinOp(left, Mult(), right):
                ...
                r = self.select_arg(right)
                res.append(Instr('movq', [l, Reg("rax")]))
                res.append(Instr('imulq', [r, Reg("rax")]))
                var = Reg('rax')
            case UnaryOp(USub(), v):
            ...
            case Call(Name('array_load'), [tup, index]):
                  tup_arg = self.select_arg(tup)
                  res.append(Instr('movq', [tup_arg, Reg('r11')]))
                  res.append(Instr('movq', [index, Reg('rax')]))
                  res.append(Instr('addq', [Immediate(1), Reg('rax')]))
                  res.append(Instr('imulq', [Immediate(8), Reg('rax')]))
                  res.append(Instr('addq', [Deref('r11',0), Reg('rax')]))
                  var = Reg('rax')
                  return var, res
            case Call(Name('len'), [tup]):
            ...
            case Call(Name('array_len'), [tup]):
                   ...
                   res.append(Instr('movq', [tup_arg, Reg('rax')])) # can we  hardcode rax ? 
                   res.append(Instr('movq', [Deref('rax',0), Reg('rax')])) 
                   #instead we can use shift left by 2 and shift right by 4 ? 
                   length_binary = "0011111111111111111111111111111111111111111111111111111111111100" # length of array 
                   res.append(Instr('andq', [Immediate(int(length_binary,2)), Reg('rax')]))
                   res.append(Instr('sarq', [Immediate(2), Reg('rax')]))
                   var = Reg('rax')
                   return var,res
            case Allocate(length, typ):
                   tag = []
            ...
            case AllocateArray(length, typ):
                   ...
                   if type(typ) == TupleType:
                        ...
                        if type(types[i]) == TupleType:
                              ...
                              tag.append('0')
                   pointer_length = '{0:060b}'.format(length)
                   tag = '1'+pointer_length+''.join(tag) #starting with 1 indicating that its the array type
                   print("Generated TAG-", tag)
                   bin_tag= int(tag, 2)
                   tag = int((bin_tag << 1) | 1) # to accomodate forwarding pointer 
                   print("Final Tag-", '{0:64b}'.format(tag) )
                   print(" obtained Tag for inst-",e," is-",tag)
                   res.append(Instr('movq', [self.getGlobal("free_ptr"), Reg('r11')]))
                   res.append(Instr('addq', [Immediate(8*(length+1)), self.getGlobal("free_ptr")]))
                   ...
                   return var, res
            case Call(Name('exit'), []):
                call_stmt = Callq("exit",255)
                res.append(call_stmt)
                var = None  # the program exits 
            case _:
              ...
              for r in regs:
                   if x != Reg(r) and type(x) != Global:
                    graph.add_edge(x, Reg(r))
                        ...
                        if x in self.unused_callee:
                             self.unused_callee.remove(x)
         return graph
    ...
    def build_interference(self, block_inst) -> UndirectedAdjList:
         ...
         for b in block_inst.keys():
            ...
            for x in mapping.keys():
                ...
                if self.isCallqInstr(x) and self.isCollectCall(x):
                         for y in mapping[x]:
              ...
              if type(v) == Reg and v in registers.keys():
                    ...
                    for adj in graph.adjacent(v):
                        ...
                        if registers[v] not in sat[adj]:
                              ...
                              pq.increase_key(adj)
                    if registers[v] in self.callee_save and registers[v] not in self.used_callee and registers[v] not in self.unused_callee:
    ...
    def checkColour(self, v, color):
         if type(v) == Variable and v.id in self.varTypes:
               return (( type(self.varTypes[v.id]) != TupleType and color not in self.tupleColors) or ( type(self.varTypes[v.id]) == TupleType and color not in self.varColors ))
         if type(v) == Deref:
         ...
         while not pq.empty():
              ...
              for x in move_graph.adjacent(v):
                   if x in color_v.keys() and color_v[x] not in sat[v] and color_v[x] >= 0:
                        ...
                        if self.checkColour(v, color_v[x]): # avoid assigning Tuple colours to other Variables
              ...
              if min_val == float("inf"):
                for x in range(0,MAX_REG):
                    if x not in sat[v] and self.checkColour(v, x):
                            min_val = x
                            break
              print("Assigned min-val:", min_val," to arg:", v," from the min range with sat mat-",sat[v])
              if min_val in callee_registers.keys():
                   if callee_registers[min_val] not in self.used_callee:
                    self.used_callee.append(callee_registers[min_val])
                   if callee_registers[min_val] in self.unused_callee:
                    self.unused_callee.remove(callee_registers[min_val])
              color_v[v] = min_val
              if type(v) == Variable and v.id in self.varTypes and type(self.varTypes[v.id]) == TupleType:
    ...
    def assign_registers(self, color_v:dict) -> dict:
         ...
         for var in color_v.keys():
              if color_v[var] in registers.keys():
                ...
                if registers[color_v[var]] in self.callee_save and registers[color_v[var]] not in self.used_callee and registers[color_v[var]] not in self.unused_callee:
                     ...
                     if type(var) == Variable and type(self.varTypes[var.id]) == TupleType:
                         ...
                         #  self.stack_space = self.stack_space + 1  # using deref that contribute to stack
                     else:
    ...
    def pe_mul(self, r1:expr, r2:expr):
         match(r1,r2):
              case (Constant(r1), Constant(r2)):
                   return Constant(mul64(r1,r2))
              case _:
                   return BinOp(r1, Mult(), r2)
    ...
    def pe_exp(self, e: expr) -> expr:
            match e:
                ...
                case Allocate(length, typ):
                    self.tuple_lengths[typ] = length  # assigning lengths to tuples for checking bounds. 
                    return e
                case AllocateArray(length, typ):
                    self.tuple_lengths[typ] = length  # assigning lengths to arrays for checking bounds. 
                    return e
                case GlobalValue(name):
                    return e 
                case _:
